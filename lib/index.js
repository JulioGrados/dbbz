'use strict'

const logDB = require('./log')
const userDB = require('./user')
const chatDB = require('./chat')
const messageDB = require('./message')
const whastsappDB = require('./whatsapp')
const metaDB = require('./meta')
const uploadDB = require('./upload')
const botDB = require('./bot')
const companyDB = require('./company')
const connectionDB = require('./connection')
const contactDB = require('./contact')
const embeddingDB = require('./embedding')
const teamDB = require('./team')
const tagDB = require('./tag')
const queueDB = require('./queue')
const trackDB = require('./track')
const pipelineDB = require('./pipeline')
const stageDB = require('./stage')
const timelineDB = require('./timeline')
const subscriptionDB = require('./subscription')
const paymentDB = require('./payment')
const scheduledMessageDB = require('./scheduledMessage')
const supportChatDB = require('./supportChat')
const supportMessageDB = require('./supportMessage')

module.exports = {
  logDB,
  userDB,
  chatDB,
  messageDB,
  whastsappDB,
  metaDB,
  uploadDB,
  botDB,
  companyDB,
  connectionDB,
  contactDB,
  embeddingDB,
  teamDB,
  tagDB,
  queueDB,
  trackDB,
  pipelineDB,
  stageDB,
  timelineDB,
  subscriptionDB,
  paymentDB,
  scheduledMessageDB,
  supportChatDB,
  supportMessageDB
}
