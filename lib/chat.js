...
const filterWithCounts = async (params) => {
  // Filtros base (sin option) - para los contadores
  const baseFilters = {
  ...
  if (params.assigned && params.assigned !== '') {
    baseFilters['assigned._id'] = new mongoose.Types.ObjectId(params.assigned);
  }
  ...
  if (params.team && params.team !== 'Seleccionar equipo') {
    baseFilters['team._id'] = new mongoose.Types.ObjectId(params.team);
  }
  if (Array.isArray(params.tags) && params.tags.length > 0) {
    const tags = params.tags.map(item => new mongoose.Types.ObjectId(item))
    baseFilters['linked.tags'] = { $in: tags };
  }
...
const filterMsgWithCounts = async (params) => {
  try {
    ...
    const matchingChatIdsAgg = await Message.aggregate([
      { $match: { text: { $regex: params.value, $options: 'i' } } },
      { $group: { _id: '$chat' } }
    ]);
