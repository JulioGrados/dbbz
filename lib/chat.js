'use strict'
const mongoose = require('mongoose')
const Chat = require('../models/chat')
const Message = require('../models/message')

const { transformParams } = require('utils').transform
// const { parseErrorDB } = require('utils').errors

const count = async params => {
  const { query } = transformParams(params)
  const count = await Chat.countDocuments(query)
  return count
}

const list = async params => {
  const { query, select, populate, sort, limit, skip } = transformParams(params)
  const chats = await Chat.find(query, select)
    .populate(populate)
    .sort(sort)
    .limit(limit)
    .skip(skip)

  return chats
}

const aggregate = async () => {
  try {
    const chats = await Chat.aggregate([
      {
        $group: {
          _id: { mobile: "$mobile", company: "$company", connection: "$connection" }, // Agrupa por mobile y company
          documents: { $push: "$$ROOT" }, // Recolecta todos los documentos de cada grupo
          count: { $sum: 1 } // Cuenta cuántos documentos hay en cada grupo
        }
      },
      {
        $match: {
          count: { $gt: 1 } // Filtra para mantener solo los grupos con más de un documento
        }
      }
    ]);
  
    return chats;
  } catch (error) {
    throw error;
  }
}

const filter = async (params) => {
  // Match sobre campos propios del chat, ANTES de los lookups:
  // permite usar el índice { company, option, updatedAt }
  const matchQuery = {
    company: new mongoose.Types.ObjectId(params.company),
    option: params.option
  }

  if (params.value && params.value.trim() !== '') {
    matchQuery.$or = [
      { mobile: { $regex: params.value, $options: 'i' } },
      { names: { $regex: params.value, $options: 'i' } }
    ]
  }
  if (params.assigned !== '') {
    matchQuery.assigned = new mongoose.Types.ObjectId(params.assigned);
  }
  if (params.connection !== 'Seleccionar conexión') {
    matchQuery.connection = new mongoose.Types.ObjectId(params.connection);
  }
  if (params.team !== 'Seleccionar equipo') {
    matchQuery.team = new mongoose.Types.ObjectId(params.team);
  }

  // linked.tags solo existe después del lookup de contacts
  const postLookupMatch = {}
  if (Array.isArray(params.tags) && params.tags.length > 0) {
    const tags = params.tags.map(item => new mongoose.Types.ObjectId(item))
    postLookupMatch['linked.tags'] = { $in: tags };
  }

  try {
    const chats = await Chat.aggregate([
      // Match primero (usa índice), lookups solo sobre los chats del tenant
      { $match: matchQuery },

      // Lookups
      {
        $lookup: {
          from: 'contacts',
          localField: 'linked',
          foreignField: '_id',
          as: 'linked'
        }
      },
      { $unwind: '$linked' },
      {
        $lookup: {
          from: 'users',
          localField: 'assigned',
          foreignField: '_id',
          as: 'assigned'
        }
      },
      { $unwind: { path: '$assigned', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: 'teams',
          localField: 'team',
          foreignField: '_id',
          as: 'team'
        }
      },
      { $unwind: { path: '$team', preserveNullAndEmptyArrays: true } },

      ...(Object.keys(postLookupMatch).length > 0 ? [{ $match: postLookupMatch }] : []),

      // Orden y paginación
      { $sort: { updatedAt: -1 } },
      { $skip: parseInt(params.page) * parseInt(params.limit) },
      { $limit: parseInt(params.limit) }
    ]);

    return chats;
  } catch (error) {
    throw error;
  }
}

/**
 * Obtener chats paginados + contadores por opción
 * Respeta todos los filtros (assigned, connection, team, tags, búsqueda)
 */
const filterWithCounts = async (params) => {
  // Filtros base (sin option) sobre campos propios del chat, ANTES de los
  // lookups: permite usar el índice { company, option, updatedAt }
  const baseFilters = {
    company: new mongoose.Types.ObjectId(params.company)
  }

  if (params.assigned && params.assigned !== '') {
    baseFilters.assigned = new mongoose.Types.ObjectId(params.assigned);
  }
  if (params.connection && params.connection !== 'Seleccionar conexión') {
    baseFilters.connection = new mongoose.Types.ObjectId(params.connection);
  }
  if (params.team && params.team !== 'Seleccionar equipo') {
    baseFilters.team = new mongoose.Types.ObjectId(params.team);
  }

  // Filtros que dependen de los lookups (contacto)
  const postLookupFilters = {}
  if (params.value && params.value.trim() !== '') {
    postLookupFilters.$or = [
      { mobile: { $regex: params.value, $options: 'i' } },
      { 'linked.names': { $regex: params.value, $options: 'i' } }
    ]
  }
  if (Array.isArray(params.tags) && params.tags.length > 0) {
    const tags = params.tags.map(item => new mongoose.Types.ObjectId(item))
    postLookupFilters['linked.tags'] = { $in: tags };
  }

  try {
    const result = await Chat.aggregate([
      // Match primero (usa índice), lookups solo sobre los chats del tenant
      { $match: baseFilters },

      // Lookups
      {
        $lookup: {
          from: 'contacts',
          localField: 'linked',
          foreignField: '_id',
          as: 'linked'
        }
      },
      { $unwind: { path: '$linked', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: 'users',
          localField: 'assigned',
          foreignField: '_id',
          as: 'assigned'
        }
      },
      { $unwind: { path: '$assigned', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: 'teams',
          localField: 'team',
          foreignField: '_id',
          as: 'team'
        }
      },
      { $unwind: { path: '$team', preserveNullAndEmptyArrays: true } },

      // Filtros que requieren campos de los lookups
      ...(Object.keys(postLookupFilters).length > 0 ? [{ $match: postLookupFilters }] : []),

      // Facet: datos paginados + contadores
      {
        $facet: {
          // Datos paginados del filtro actual
          data: [
            { $match: { option: params.option } },
            { $sort: { updatedAt: -1 } },
            { $skip: parseInt(params.page) * parseInt(params.limit) },
            { $limit: parseInt(params.limit) }
          ],
          // Contador: Abiertos
          countAbiertos: [
            { $match: { option: 'abiertos' } },
            { $count: 'count' }
          ],
          // Contador: Resueltos
          countResueltos: [
            { $match: { option: 'resueltos' } },
            { $count: 'count' }
          ]
        }
      }
    ]);

    return {
      chats: result[0].data,
      counts: {
        abiertos: result[0].countAbiertos[0]?.count || 0,
        resueltos: result[0].countResueltos[0]?.count || 0
      }
    };
  } catch (error) {
    throw error;
  }
}

const filterMsg = async (params) => {
  try {
    // Paso 1: obtener los IDs de los chats que tienen coincidencias en message.text
    // Filtrado por company: evita escanear mensajes de otros tenants
    const matchingChatIdsAgg = await Message.aggregate([
      {
        $match: {
          company: new mongoose.Types.ObjectId(params.company),
          text: { $regex: params.value, $options: 'i' }
        }
      },
      {
        $group: {
          _id: '$chat'
        }
      }
    ]);

    const matchingChatIds = matchingChatIdsAgg.map(item => item._id);

    if (matchingChatIds.length === 0) {
      return []; // No hay coincidencias
    }

    // Paso 2: armar filtro principal
    const matchQuery = {
      _id: { $in: matchingChatIds },
      company: new mongoose.Types.ObjectId(params.company),
      option: params.option
    };

    if (params.assigned !== '') {
      matchQuery.assigned = new mongoose.Types.ObjectId(params.assigned);
    }

    if (params.connection !== 'Seleccionar conexión') {
      matchQuery.connection = new mongoose.Types.ObjectId(params.connection);
    }

    if (params.team !== 'Seleccionar equipo') {
      matchQuery.team = new mongoose.Types.ObjectId(params.team);
    }

    const pipeline = [
      { $match: matchQuery },

      // Lookups para enriquecer los chats
      {
        $lookup: {
          from: 'contacts',
          localField: 'linked',
          foreignField: '_id',
          as: 'linked'
        }
      },
      { $unwind: '$linked' },

      {
        $lookup: {
          from: 'users',
          localField: 'assigned',
          foreignField: '_id',
          as: 'assigned'
        }
      },
      { $unwind: { path: '$assigned', preserveNullAndEmptyArrays: true } },

      {
        $lookup: {
          from: 'teams',
          localField: 'team',
          foreignField: '_id',
          as: 'team'
        }
      },
      { $unwind: { path: '$team', preserveNullAndEmptyArrays: true } },

      // Filtro opcional por tags
      ...(Array.isArray(params.tags) && params.tags.length > 0
        ? [
            {
              $match: {
                'linked.tags': {
                  $in: params.tags.map(tag => new mongoose.Types.ObjectId(tag))
                }
              }
            }
          ]
        : []),

      // Orden y paginación
      { $sort: { updatedAt: -1 } },
      { $skip: parseInt(params.page) * parseInt(params.limit) },
      { $limit: parseInt(params.limit) },

      // Selección de campos a retornar
      {
        $project: {
          _id: 1,
          updatedAt: 1,
          createdAt: 1,
          assigned: 1,
          team: 1,
          connection: 1,
          linked: {
            _id: '$linked._id',
            names: '$linked.names',
            mobile: '$linked.mobile',
            mobileCode: '$linked.mobileCode',
            username: '$linked.username',
            email: '$linked.email',
            firstName: '$linked.firstName',
            lastName: '$linked.lastName',
            photo: '$linked.photo',
            tags: '$linked.tags'
          },
          mobile: 1,
          names: 1,
          mobileCode: 1,
          lastMessage: 1,
          previousMessage: 1,
          previousMessageType: 1,
          lastChannel: 1,
          lastMessageType: 1,
          lastMessageStatus: 1,
          read: 1,
          count: 1,
          phoneNoId: 1,
          auto: 1,
          country: 1,
          date: 1,
          create: 1,
          channel: 1,
          company: 1,
          bot: 1,
          name: 1,
          transmitter: 1,
          option: 1,
          status: 1
        }
      }
    ];

    const chats = await Chat.aggregate(pipeline);
    return chats;
  } catch (error) {
    console.error('Error en filterMsg:', error);
    throw error;
  }
};

const filterMsgWithCounts = async (params) => {
  try {
    // Paso 1: obtener IDs de chats con coincidencias en message.text
    // Filtrado por company: evita escanear mensajes de otros tenants
    const matchingChatIdsAgg = await Message.aggregate([
      { $match: { company: new mongoose.Types.ObjectId(params.company), text: { $regex: params.value, $options: 'i' } } },
      { $group: { _id: '$chat' } }
    ]);

    const matchingChatIds = matchingChatIdsAgg.map(item => item._id);

    if (matchingChatIds.length === 0) {
      return { chats: [], counts: { abiertos: 0, resueltos: 0 } };
    }

    // Paso 2: filtro base sin option (para poder contar abiertos y resueltos)
    const baseMatchQuery = {
      _id: { $in: matchingChatIds },
      company: new mongoose.Types.ObjectId(params.company),
    };

    if (params.assigned && params.assigned !== '') {
      baseMatchQuery.assigned = new mongoose.Types.ObjectId(params.assigned);
    }
    if (params.connection && params.connection !== 'Seleccionar conexión') {
      baseMatchQuery.connection = new mongoose.Types.ObjectId(params.connection);
    }
    if (params.team && params.team !== 'Seleccionar equipo') {
      baseMatchQuery.team = new mongoose.Types.ObjectId(params.team);
    }

    const tagsFilter = Array.isArray(params.tags) && params.tags.length > 0
      ? [{ $match: { 'linked.tags': { $in: params.tags.map(tag => new mongoose.Types.ObjectId(tag)) } } }]
      : [];

    const projectFields = {
      $project: {
        _id: 1, updatedAt: 1, createdAt: 1, assigned: 1, team: 1, connection: 1,
        linked: {
          _id: '$linked._id', names: '$linked.names', mobile: '$linked.mobile',
          mobileCode: '$linked.mobileCode', username: '$linked.username',
          email: '$linked.email', firstName: '$linked.firstName',
          lastName: '$linked.lastName', photo: '$linked.photo', tags: '$linked.tags'
        },
        mobile: 1, names: 1, mobileCode: 1, lastMessage: 1, previousMessage: 1,
        previousMessageType: 1, lastChannel: 1, lastMessageType: 1,
        lastMessageStatus: 1, read: 1, count: 1, phoneNoId: 1, auto: 1,
        country: 1, date: 1, create: 1, channel: 1, company: 1, bot: 1,
        name: 1, transmitter: 1, option: 1, status: 1
      }
    };

    const pipeline = [
      { $match: baseMatchQuery },
      { $lookup: { from: 'contacts', localField: 'linked', foreignField: '_id', as: 'linked' } },
      { $unwind: '$linked' },
      { $lookup: { from: 'users', localField: 'assigned', foreignField: '_id', as: 'assigned' } },
      { $unwind: { path: '$assigned', preserveNullAndEmptyArrays: true } },
      { $lookup: { from: 'teams', localField: 'team', foreignField: '_id', as: 'team' } },
      { $unwind: { path: '$team', preserveNullAndEmptyArrays: true } },
      ...tagsFilter,
      {
        $facet: {
          data: [
            { $match: { option: params.option } },
            { $sort: { updatedAt: -1 } },
            { $skip: parseInt(params.page) * parseInt(params.limit) },
            { $limit: parseInt(params.limit) },
            projectFields
          ],
          countAbiertos: [
            { $match: { option: 'abiertos' } },
            { $count: 'count' }
          ],
          countResueltos: [
            { $match: { option: 'resueltos' } },
            { $count: 'count' }
          ]
        }
      }
    ];

    const result = await Chat.aggregate(pipeline);
    return {
      chats: result[0].data,
      counts: {
        abiertos: result[0].countAbiertos[0]?.count || 0,
        resueltos: result[0].countResueltos[0]?.count || 0
      }
    };
  } catch (error) {
    console.error('Error en filterMsgWithCounts:', error);
    throw error;
  }
};

// , select: 'lastMenssage lastChannel updatedAt linked'
const create = async body => {
  try {
    const chat = await Chat.create(body)
    return await Chat.findById(chat._id).populate([
      { path: 'linked assigned team' }
    ])
  } catch (errorDB) {
    // const error = parseErrorDB(errorDB)
    throw errorDB
  }
}

const update = async (chatId, body) => {
  const chat = await Chat.findOne({ _id: chatId })
  if (chat === null) {
    const error = {
      status: 404,
      chat: 'El chat no se encontro'
    }
    throw error
  }
  
  try {
    if (body.statusRead) {
      await Chat.collection.updateOne(
        { _id: mongoose.Types.ObjectId(chatId) },
        { $set: {read: true, count: 0} }
      )
      const chatUpdate = await Chat.findById(chatId).populate([
        { path: 'linked' },
        { path: 'assigned' },
        { path: 'team' },
        { path: 'tags' }
      ]);
      console.log('chatUpdate', chatUpdate)
      return chatUpdate  
    } else if (body.statusNotRead) {
      await Chat.collection.updateOne(
        { _id: mongoose.Types.ObjectId(chatId) },
        { $set: {read: false, count: 0} }
      )
      const chatUpdate = await Chat.findById(chatId).populate([
        { path: 'linked' },
        { path: 'assigned' },
        { path: 'team' },
        { path: 'tags' }
      ]);
      console.log('chatUpdate', chatUpdate)
      return chatUpdate  
    }else if (body.only) {
      const chatUpdate = await Chat.findOneAndReplace(
        {
          _id: chatId
        },
        {
          ...chat.toJSON(),
          ...body
        },
        {
          new: true
        }
      ).populate(
      [
        { path: 'linked' },
        { path: 'assigned' },
        { path: 'team'},
        { path: 'tags'},
      ])
      console.log('chatUpdate', chatUpdate)
      return chatUpdate 
    } else if (body.isNotChangeTime) {
      await Chat.collection.updateOne(
        { _id: mongoose.Types.ObjectId(chatId) },
        { $set: {stage: mongoose.Types.ObjectId(body.stage), pipeline: mongoose.Types.ObjectId(body.pipeline)} }
      )
      const chatUpdate = await Chat.findById(chatId).populate([
        { path: 'linked' },
        { path: 'assigned' },
        { path: 'team' },
        { path: 'tags' }
      ]);
      console.log('chatUpdate', chatUpdate)
      return chatUpdate 
    } else {
      
      const chatUpdate = await Chat.findOneAndUpdate({ _id: chatId }, body, { new: true, }).populate(
        [
          { path: 'linked' },
          { path: 'assigned' },
          { path: 'team'},
          { path: 'tags'},
        ])
      return chatUpdate  
    }
    
  } catch (errorDB) {
    // const error = parseErrorDB(errorDB)
    throw errorDB
  }
}

const updateMany = async (query, body) => {
  try {
    const chats = await Chat.updateMany(query, { "$set": { ...body } })
    return chats
  } catch (errorDB) {
    // const error = parseErrorDB(errorDB)
    throw errorDB
  }
}

const updateManyTime = async (stage, targetStageId) => {
  try {
    await Chat.collection.updateMany(
      { stage: mongoose.Types.ObjectId(stage) },
      { $set: {stage: mongoose.Types.ObjectId(targetStageId)} }
    )
  } catch (errorDB) {
    throw errorDB
  }
}

const updateManyOther = async (query, body) => {
  try {
    const chats = await Chat.updateMany(query, body)
    return chats
  } catch (errorDB) {
    // const error = parseErrorDB(errorDB)
    throw errorDB
  }
}

const detail = async params => {
  const { query, select, populate } = transformParams(params)
  let chat
  try {
    chat = await Chat.findOne(query, select).populate(populate)
  } catch (errorDB) {
    // const error = parseErrorDB(errorDB)
    throw errorDB
  }

  if (chat === null) {
    const error = {
      status: 404,
      menssage: 'El chat no se encontro'
    }

    throw error
  }

  return chat
}

const detailOnly = async params => {
  try {
    const { query, select, populate } = transformParams(params) || {}

    if (!query) {
      console.warn('No se proporcionó una query válida en transformParams')
      return null
    }

    let queryBuilder = Chat.findOne(query, select)

    if (populate) {
      queryBuilder = queryBuilder.populate(populate)
    }

    const chat = await queryBuilder.exec()

    return chat || null
  } catch (error) {
    console.error('Error en detailOnly:', error)
    return null
  }
}

const remove = async chatId => {
  const chat = Chat.findOne({ _id: chatId })

  if (chat === null) {
    const error = {
      status: 404,
      chat: 'El chat no se encontro'
    }

    throw error
  }

  try {
    const chatRemove = Chat.deleteOne({ _id: chatId })
    return chatRemove
  } catch (errorDB) {
    // const error = parseErrorDB(errorDB)
    throw errorDB
  }
}

const removeMany = async query => {
  
  try {
    const chats = await Chat.deleteMany(query)

    return chats
  } catch (errorDB) {
    // const error = parseErrorDB(errorDB)
    throw errorDB
  }
}


module.exports = {
  count,
  list,
  create,
  update,
  filter,
  filterWithCounts,
  filterMsg,
  filterMsgWithCounts,
  updateMany,
  updateManyTime,
  updateManyOther,
  detail,
  detailOnly,
  remove,
  aggregate,
  removeMany
}
